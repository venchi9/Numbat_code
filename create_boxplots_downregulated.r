...
samples <- c("PDCD4", "EIF4EBP1", "EIF2AK2", "EIF2S1", "TP53", "FAS", "BAX", "PCBP2", "RPS2", "RPS15", "RPL23A", "RPL35A", "RPS15", "RPS24", "PSMB8", "PSMB9", "TAP1", "TYK2", "STAT1", "STAT2", "JAK2", "IRF3", "MX1", "MX2", "ISG15")
#n = 60
...
pdf(str_glue("{fig_dir}/boxplot_downregulated_{sample_name}_witht_test.pdf"), width=4, height=4)
...
p <- ggplot(combined, aes( x = factor(Group, level = level_order), y = expression)) +
 ...
 p + stat_compare_means(comparisons = my_comparisons, method = "t.test", label = "p.format") + theme_bw()+ scale_fill_manual(values = c("steelblue1", "royalblue4"))
 dev.off()

print("plot without legends")
 pdf(str_glue("{fig_dir}/boxplot_downregulated_{sample_name}_nolegends.pdf"), width=3, height=3)
level_order<-c('Primary', 'LN')
my_comparisons<-list(c("Primary", "LN"))
p <- ggplot(combined, aes( x = factor(Group, level = level_order), y = expression)) +
  geom_boxplot(aes(fill = Group)) + ylim(-5, 6)
 p  + theme_bw() + NoLegend() + theme(axis.title.x = element_blank(),
          axis.title.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())+ scale_fill_manual(values = c("steelblue1", "royalblue4"))
 dev.off()