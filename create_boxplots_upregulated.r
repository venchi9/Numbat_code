...
samples <- c("EIF4E", "EIF4G1", "PPP1R15A", "PPP1CA", "RB1", "HIF1A", "VEGFA", "PCBP1", "RPL36A", "JAK1")
#n = 60
...
pdf(str_glue("{fig_dir}/boxplot_upregulated_{sample_name}_witht_test.pdf"), width=4, height=4)
...
p <- ggplot(combined, aes( x = factor(Group, level = level_order), y = expression)) +
 ...
 p + stat_compare_means(comparisons = my_comparisons, method = "t.test", label = "p.format") + theme_bw()+ scale_fill_manual(values = c("tomato1", "red4"))
 dev.off()

print("plot without legends")
 pdf(str_glue("{fig_dir}/boxplot_upregulated_{sample_name}_nolegends.pdf"), width=3, height=3)
level_order<-c('Primary', 'LN')
my_comparisons<-list(c("Primary", "LN"))
p <- ggplot(combined, aes( x = factor(Group, level = level_order), y = expression)) +
  geom_boxplot(aes(fill = Group)) + ylim(-5, 6)
 p  + theme_bw() + NoLegend() + theme(axis.title.x = element_blank(),
          axis.title.y = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())+ scale_fill_manual(values = c("tomato1", "red4"))
 dev.off()