#Take the new transcriptional/clonal groups and do the walter plots on them
...
#Plot
pdf(str_glue("{fig_dir}/{sample_name}_{comparison_name}_gsa_barplot.pdf"), width=11.6, height=11.6)
gsa_plot()
dev.off()

pdf(str_glue("{fig_dir}/{sample_name}_{comparison_name}_gsa_barplot_test.pdf"), width=11.6, height=8.2)
gsa_plot()
dev.off()
...
test<-subset(plot_data, gene_set == "SRP-dependent cotranslational protein targeting to membrane")
...
#Save out
write.csv(highFCgenes, file = str_glue("{data_dir}/{sample_name}_{comparison_name}_SRP-dependent cotranslational protein targeting to membrane.csv"))
